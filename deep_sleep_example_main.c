...
void app_main(void)
{
    ...
    // Configure BUTTON_GPIO as an input pin
    io_conf.intr_type = GPIO_INTR_DISABLE; // No interrupt needed
    io_conf.pin_bit_mask = 1ULL << BUTTON_GPIO;
    io_conf.mode = GPIO_MODE_INPUT;
    io_conf.pull_up_en = GPIO_PULLUP_ENABLE; // Assuming you're using a pull-up configuration
    gpio_config(&io_conf);

    // Configure LED_GPIO as an output pin
    gpio_set_direction(LED_GPIO, GPIO_MODE_OUTPUT);
    
    switch (esp_sleep_get_wakeup_cause()) {
#if CONFIG_EXAMPLE_EXT0_WAKEUP              //---------------------------------------------------------------------
        case ESP_SLEEP_WAKEUP_EXT0: {
            struct timeval call1;
            gettimeofday(&call1, NULL);
            printf("Wake up from ext0.\n");
            while(1){
                struct timeval now;
                gettimeofday(&now, NULL);
                int count = (now.tv_sec - call1.tv_sec) * 1000 + (now.tv_usec - call1.tv_usec) / 1000;
                if( count < 2000 && gpio_get_level(39) == 1){
                    printf("Entering into deep sleep agian due to not holding button for %dms.\n", count);
                ...
                else if(count >= 2000 && gpio_get_level(39) == 0) {
                //else {
                    printf("Button got pressed for %dms.\n", count);
                    printf("Wake up from button. Time spent in deep sleep: %dms\n", sleep_time_ms);
    ...
    while (1)
    {
        if(gpio_get_level(BUTTON_GPIO) == 0)
        {
            ...
            while(1){
                ...
                gettimeofday(&now, NULL);
                int count = (now.tv_sec - call1.tv_sec) * 1000 + (now.tv_usec - call1.tv_usec) / 1000;
                if( count >= 2000 && gpio_get_level(BUTTON_GPIO) == 0){
                    printf("Entering into deep sleep due to holding button for %dms.\n", count);
                    vTaskDelay(1000 / portTICK_PERIOD_MS);
                    gettimeofday(&sleep_enter_time, NULL);
                    esp_deep_sleep_start();
                }
                else if(gpio_get_level(BUTTON_GPIO) == 1) {
                    printf("Button got release before %dms holdings.\n", count);
                    break;
                }    
            }
        }
